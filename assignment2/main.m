...
axis_limit=[-0.5, 0.5, -0.5, 0.5, 0, 0.5];
figure
for i = 1:numberOfSteps
    %-------------------MOVE----------------------%
    biTei = GetDirectGeometry(qstep1,geom_model,linkType);
    for j=1:numberOfLinks
...
for i = 1:numberOfSteps
%-------------------MOVE----------------------%
biTei = GetDirectGeometry(qstep2,geom_model,linkType);
for j=1:numberOfLinks
    r(:,j) = GetBasicVectorWrtBase(biTei, j);
    plot3(r(1,:),r(2,:),r(3,:),'-o','Color','b', 'MarkerSize',5,'LineWidth',1.5)
end
...
for i = 1:numberOfSteps
%-------------------MOVE----------------------%
biTei = GetDirectGeometry(qstep3,geom_model,linkType);
for j=1:numberOfLinks
    r(:,j) = GetBasicVectorWrtBase(biTei, j);
    plot3(r(1,:),r(2,:),r(3,:),'-o','Color','b', 'MarkerSize',5,'LineWidth',1.5)
...
% %1.5
axislimits=[-0.5, 1.5, -0.5, 3, -0.9, 0.5];
figure
for k=1:numberOfLinks
    ...
    for i = 1:numberOfSteps
        %-------------------MOVE----------------------%
        biTei = GetDirectGeometry(qstep,geom_model,linkType);
        for j=1:numberOfLinks
            r(:,j) = GetBasicVectorWrtBase(biTei, j);
            plot3(r(1,:),r(2,:),r(3,:),'-o','Color','b', 'MarkerSize',5,'LineWidth',1.5)
...
axislimits=[-0.5, 0.5, -0.5, 0.5, 0, 0.5];
figure
for k=1:numberOfLinks
    qstep=zeros(1,numberOfLinks);
...
axislimits=[-0.9, 0.9, -0.8, 0.8, 0, 1];
figure
for k=1:numberOfLinks
    qstep=zeros(1,numberOfLinks);
